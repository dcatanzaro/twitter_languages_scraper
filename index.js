                ...
                tweetSplit.map((tw, index) => {
                    ...
                    if (split) {
                        if (tw.includes(["first language"])) {
                            tweetSanitized[0] = split.trim();
                        } else if (tw.includes(["had difficulties"])) {
                            tweetSanitized[1] = split.trim();
                        } else if (tw.includes(["most used"])) {
                            tweetSanitized[2] = split.trim();
                        } else if (tw.includes(["totally hate"])) {
                            tweetSanitized[3] = split.trim();
                        } else if (tw.includes(["most loved"])) {
                            tweetSanitized[4] = split.trim();
                        } else if (tw.includes(["for beginners"])) {
                            tweetSanitized[5] = split.trim();
                        }
                    }
                });

                tweetSanitized.map((language, index) => {
                    const indexLanguage = arLanguages.indexOf(language);
                    const sanitizedLanguage =
                        indexLanguage > -1 ? arLanguages[indexLanguage] : null;

                    if (sanitizedLanguage && index < 6) {
                        if (typeof arResults[index][language] !== "undefined") {
                            arResults[index][language]++;
                        } else {
                            arResults[index][language] = 1;
                        }
                    }
                });
